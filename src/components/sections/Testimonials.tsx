...
export async function Testimonials() {
  ...
  return (
    <section id="testimonials" className="py-24 bg-gradient-to-b from-black via-[#0a0a0a] to-black relative overflow-hidden">
        ...
        <div className="mt-16 grid md:grid-cols-3 gap-8">
          {testimonials.length === 0 ? (
            <div className="col-span-3 text-center py-12">
              <p className="text-white/60">No testimonials yet. Add some in the Sanity Studio!</p>
            </div>
          ) : (
            testimonials.map((testimonial, index) => (
              <FadeIn key={testimonial._id} delay={index * 0.1}>
                <div className="bg-white/5 backdrop-blur-sm p-8 rounded-xl border border-white/10 hover:border-primary/50 hover:bg-white/10 transition-all h-full flex flex-col">
                  {/* Quote icon */}
                  ...
                  <div className="border-t border-white/10 pt-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="font-semibold text-white mb-1">{testimonial.clientName}</p>
                        <p className="text-sm text-white/60">{testimonial.clientRole}</p>
